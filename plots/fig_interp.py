...
def main():
    ...
    axL.scatter(path_marks[:, 0], path_marks[:, 1], color=C.ACCENT, s=42,
    ...
    axR.pcolormesh(xx, yy, dens, cmap=C.DENSITY_CMAP, vmin=0,
                   vmax=np.percentile(dens, 99.5), shading="auto", rasterized=True)
    for ln in h_lines:
        p = push(ln); axR.plot(p[:, 0], p[:, 1], color="#1D4ED8", lw=0.7, alpha=0.7)
    for ln in v_lines:
        p = push(ln); axR.plot(p[:, 0], p[:, 1], color="#1D4ED8", lw=0.7, alpha=0.7)
    pp = push(path)
    axR.plot(pp[:, 0], pp[:, 1], color=C.ACCENT, lw=2.4, zorder=5)
    ppm = push(path_marks)
    axR.scatter(ppm[:, 0], ppm[:, 1], color=C.ACCENT, s=42, zorder=6,
                edgecolor="white", linewidth=1.0)
    axR.set_title("Data space  x = f(z)   (over learned density)",
    ...
    fig.suptitle("A normalizing flow is a learned, invertible warp of space",
    ...
    fig.text(0.012, 0.012,
             ...
             "smooth interpolation path along the data distribution.",
