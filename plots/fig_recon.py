...
def main():
    ...
    means = []
    for ds in DATASETS:
        mm = C.load_cache(ds, "realnvp")["model"]; mm.eval()
        err = C.reconstruction_error(mm, C.get_dataset(ds, 3000))
        err = np.clip(err, 1e-12, None)
        axR.hist(np.log10(err), bins=45, histtype="step", linewidth=2.0,
                 color=DS_COLOR[ds], label=f"{C.DATASETS[ds][0]} (mean {err.mean():.1e})")
        means.append(err.mean())
    axR.axvline(np.log10(EPS32), color=C.ACCENT, linestyle="--", linewidth=1.4)
    axR.text(np.log10(EPS32), axR.get_ylim()[1] * 0.96, " float32 epsilon",
             color=C.ACCENT, fontsize=9, va="top", ha="left")
    axR.set_xlabel(r"$\log_{10}\;|x - f(f^{-1}(x))|$  (sum over dims)")
    axR.set_ylabel("count")
    axR.set_title("Reconstruction error sits at machine precision",
                  fontsize=13, color=C.AGENT, loc="left", fontweight="bold")
    axR.legend(loc="upper left", frameon=False, fontsize=8.5)
    C.style_axes(axR, grid_axis="y")

    fig.suptitle("Real NVP is invertible by construction",
    ...
    fig.text(0.012, 0.012,
             ...
             "the property the correctness tests verify.",
