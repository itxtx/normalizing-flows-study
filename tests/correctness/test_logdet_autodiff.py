...
def compute_jacobian_logdet(flow, x, direction="forward"):
    ...
    for i in range(batch_size):
        x_single = x[i].detach().requires_grad_(True)
        
        def flow_fn(input_tensor):
            if direction == "forward":
                output, _ = flow.forward(input_tensor.unsqueeze(0))
            else:
                output, _ = flow.inverse(input_tensor.unsqueeze(0))
            return output.squeeze(0)  # Remove batch dimension for jacobian computation

        jacobian = torch.autograd.functional.jacobian(flow_fn, x_single)
    ...
    def test_logdet_vs_autodiff(self, dim, direction):
        ...
        for flow in flows:
            ...
            try:
                ...
                assert valid_mask.all(), f"Non-finite log-determinant for {type(flow).__name__}"
                    ...
                    if use_relative.any():
                        ...
                        if max_rel_error > 5e-4:
                            pytest.fail(f"**critical-bug** Inverse log-det consistency with autodiff failed for {type(flow).__name__} "
                                      f"(dim={dim}): max relative error = {max_rel_error:.2e} > 5e-4")
